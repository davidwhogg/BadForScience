...
\newcommand{\draftstatus}{\textcolor{red}{Draft version 2024-01-02}}
...
For example, HOGG CITES. HOGG: NOTE that we are self-citing here; we are guilty ourselves!
...
HOGG: Note that you could solve this with fully explainable methods.

HOGG: Note that you can't solve this by doing finitely many tests of the system, by, say, simulating fake data sets.
...
HOGG: TAKE A PARAGRAPH TO EXPLAIN THIS CLAIM, and make a figure: Given the large size of the test set, the precision of the result is high: This bias in the estimated relationship is larger than 30-sigma in the worst data points.

HOGG: COMMENT AND SHOW that the bias goes down as the information in the features goes up (relative to the information in the training-set distribution (the prior)). Add this to the new figure.

HOGG COMMENT AND SHOW that nothing here depends on the ML model complexity. Add this to the new figure.
